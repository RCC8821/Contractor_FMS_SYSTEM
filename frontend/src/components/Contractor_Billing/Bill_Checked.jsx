...
const Bill_Checked = () => {
  ...
  const handleNext = () => {
    ...
    const allMatchingRecords = billsData.filter(item =>
    ...
    console.log('📋 UIDs Found:', allMatchingRecords.map(r => r.UID));
    ...
    setEditableData(allMatchingRecords.map(item => ({
      uid: item.UID,
      areaQuantity2: '',
      unit2: '',
      qualityApprove2: '',
            ...
            <div className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input value={formData.projectName} readOnly className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg bg-gray-50" />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  {formData.projectId ? (
                    <p className="text-xs text-gray-500 mt-1">
                      {contractorOptions.length} unique contractors for this project
                    </p>
                  ) : (
                    <p className="text-xs text-red-500 mt-1">⚠️ पहले Project ID चुनें</p>
                  )}
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Contractor Name</label>
                  <input value={formData.contractorName} readOnly className="w-full px-4 py-3 border-2 border-gray-200 rounded-lg bg-gray-50" />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  ...
                  {formData.contractorFirm ? (
                    ...
                    <p className="text-xs text-red-500 mt-1">⚠️ पहले Contractor चुनें</p>
                  )}
                </div>
        ...
        {step === 2 && (
          <div className="bg-white rounded-lg shadow-md p-8">
            ...
            <div className="bg-yellow-50 border border-yellow-300 rounded-lg p-6 mb-8">
              <h3 className="text-lg font-bold text-yellow-900 mb-4">Global Inputs (Apply to All UIDs)</h3>
              <div className="grid md:grid-cols-3 gap-6">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    Attendance Sheet <span className="text-gray-400">(Optional)</span>
                  </label>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 border">#</th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 border">UID</th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 border">Work Name</th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 border">Work Description</th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 border">Area/Qty 2</th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 border">Unit 2</th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 border">Quality 2</th>
                    <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 border">Photo</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredData.map((item, i) => (
                    <tr key={`${item.UID}-${i}`} className="hover:bg-blue-50">
                      <td className="px-4 py-4 text-sm font-bold text-gray-600 border">{i + 1}</td>
                      <td className="px-4 py-4 font-semibold text-indigo-600 border">{item.UID}</td>
                      ...
                      <td className="px-4 py-4 border">
                        <input type="number" value={editableData[i]?.areaQuantity2 || ''} onChange={e => handleInputChange(i, 'areaQuantity2', e.target.value)} className="w-full px-3 py-2 border rounded-md" placeholder="0" />
                      </td>
                      <td className="px-4 py-4 border">
                      ...
                      <td className="px-4 py-4 border">
                        ...
                        {editableData[i]?.photoEvidenceFile && <p className="text-xs text-green-600 mt-1 flex items-center gap-1"><CheckCircle size={12} /> {editableData[i].photoEvidenceFile.name}</p>}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="flex justify-end mt-6">
              <button
                ...
                disabled={isSaving || !globalFiles.measurementSheet || !globalStatus}
