...
//     const c = projectResponse.columns;

//     const ids = c.Project_ID || [];
//     const names = c.Project_Name || [];
//     const engineers = c.Site_Engineer_Name || [];
//     const contNames = c.Contractor_Name || [];
//     const contFirms = c.Contractor_Firm_Name || [];
//     const works = c.Contractor_Work_Name || [];
//     const units = c.Single_Unit || [];

//     const options = [];
//     for (let i = 0; i < ids.length; i++) {
//       const id = (ids[i] || '').toString().trim();
//       if (!id) continue;

//       options.push({
//         projectId: id,
//         projectName: (names[i] || '').trim(),
//         siteEngineerName: (engineers[i] || '').trim(),
//         contractorName: (contNames[i] || '').trim(),
//         contractorFirmName: (contFirms[i] || '').trim(),
//         contractorWorkName: (works[i] || '').trim(),
//         unit: (units[i] || '').trim(),
//       });
//     }
//     return options;
...
//   // Auto-fill Contractor Firm Name - FIXED LOGIC (Direct column mapping, handles all cases including blank Project_ID)
//   useEffect(() => {
//     if (!shared.contractorName || !projectResponse?.columns) {
//       setShared(p => ({ ...p, contractorFirmName: '' }));
//       return;
//     }

//     const { Contractor_Name = [], Contractor_Firm_Name = [] } = projectResponse.columns;

//     for (let i = 0; i < Contractor_Name.length; i++) {
//       if (String(Contractor_Name[i] || '').trim() === shared.contractorName.trim()) {
//         const firm = String(Contractor_Firm_Name[i] || '').trim();
//         if (firm) {
//           setShared(p => ({ ...p, contractorFirmName: firm }));
//           return; // First non-empty match milte hi set kar do
//         }
//       }
//     }

//     // Agar koi firm nahi mila
//     setShared(p => ({ ...p, contractorFirmName: '' }));
//   }, [shared.contractorName, projectResponse]);
...
//         remark: w.remark,
//         billNo: shared.billNo,
//         billDate: shared.billDate,
//       }))
//     );
...
//       contractorWorkName: 'Contractor_Work_Name',
//       unit: 'Single_Unit'
//     };

//     const key = map[field];
//     if (!key || !projectResponse.columns[key]) return [];

//     return [...new Set(
//       projectResponse.columns[key]
...
const Bill_Tally_form = () => {
  ...
  // Auto-fill Contractor Name when Contractor Firm is selected
  useEffect(() => {
    if (!shared.contractorFirmName || !projectResponse?.columns) {
      setShared(p => ({ ...p, contractorName: '' }));
      return;
    }

    const { Contractor_Name = [], Contractor_Firm_Name = [] } = projectResponse.columns;

    for (let i = 0; i < Contractor_Firm_Name.length; i++) {
      if (String(Contractor_Firm_Name[i] || '').trim() === shared.contractorFirmName.trim()) {
        const name = String(Contractor_Name[i] || '').trim();
        if (name) {
          setShared(p => ({ ...p, contractorName: name }));
          return;
        }
      }
    }
    ...
    setShared(p => ({ ...p, contractorName: '' }));
  }, [shared.contractorFirmName, projectResponse]);
  ...
  const getColumnValues = (field) => {
    ...
    const map = {
      contractorName: 'Contractor_Name',
      contractorFirmName: 'Contractor_Firm_Name',
      contractorWorkName: 'Contractor_Work_Name',
      unit: 'Single_Unit'
    };
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <InputWithIcon
                ...
                label="Project Name *"
                value={shared.projectName}
                readOnly
              />
              <InputWithIcon
                icon={<HardHat className="w-5 h-5 text-yellow-600" />}
                label="Site Engineer *"
                value={shared.siteEngineerName}
                readOnly
              />
            </div>

            {/* Contractor Firm (Dropdown) & Contractor Name (Auto-fill) */}
            <div className="grid md:grid-cols-2 gap-6">

              {/* Contractor Firm → DROPDOWN */}
              <ReusableDropdown
                label="Contractor Firm *"
                value={shared.contractorFirmName}
                onSelect={v => setShared(p => ({ ...p, contractorFirmName: v }))}
                options={getColumnValues('contractorFirmName')}
                placeholder="Select Contractor Firm"
                icon={<Building2 className="w-5 h-5 text-purple-600" />}
              />

              {/* Contractor Name → AUTO-FILL INPUT */}
              <InputWithIcon
                icon={<User className="w-5 h-5 text-green-600" />}
                label="Contractor Name *"
                value={shared.contractorName}
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="truncate">
          {value
            ? `${value} → ${options.find(o => o.projectId === value)?.projectName || ''}`
            : 'Select Project'}
        </span>
        <ChevronDown className={`w-5 h-5 transition ${open ? 'rotate-180' : ''}`} />
      </button>
