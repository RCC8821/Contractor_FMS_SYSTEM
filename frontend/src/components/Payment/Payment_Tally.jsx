...
// //                 val = "0.00";
// //             } else {
// //                 // Round Up field ke liye limit -9 to +9
...
// //             const displayBalance = balanceAmount;
...
// //                 balanceAmount8: calc.balanceAmount || 0, // ← RoundUp ke saath jaayega
...
//         // Dropdown, Date, or non-numeric fields
//         if (options !== null || isDate || !isNumeric) {
//             setLocalValue(value);
//             if (options !== null) onValueChange(id, value);
//             return;
...
//         // Round Up field ke liye special handling - 2 decimal places tak allow karo
//         if (label.includes("Round Up")) {
//             // Allow: optional minus, digits, optional decimal point with up to 2 decimal places
//             // Examples: -0.20, 0.30, -.40, .50, -5.75, 9.00
//             if (/^-?\d*\.?\d{0,2}$/.test(value) || value === '' || value === '-' || value === '.' || value === '-.') {
//                 setLocalValue(value);
//             }
//             return;
//         }

//         // Normal numeric fields ke liye
//         if (/^-?[\d.]*$/.test(value)) {
//             setLocalValue(value);
//         }
//     };
...
//         // Empty, single minus, or single decimal ko 0.00 banao
//         if (val === "" || val === "-" || val === "." || val === "-.") {
//             val = "0.00";
//         } else {
//             let num = parseFloat(val);
...
//                 val = "0.00";
//             } else {
//                 // Round Up field ke liye -9 to +9 limit with 2 decimal places
//                 if (label.includes("Round Up")) {
//                     num = Math.max(-9, Math.min(9, num));
//                     val = num.toFixed(2);
//                 } else {
...
import { 
  ...
  useGetBankNamesQuery  // New import
} from "../../features/Payment/Payment_Tally_Slice";
import { X, CheckSquare, Square, FileText, ChevronDown } from "lucide-react";
...
const PaymentInputComponent = ({
    ...
    const isDropdown = options !== null;
    const inputStyle = `w-full text-lg font-semibold border-b-2 outline-none py-1 
        transition duration-150
        ${color === "blue" 
            ? "border-blue-300 focus:border-blue-600 text-blue-700" 
            : color === "green" 
        ...
        <div className="bg-white p-3 rounded-lg border border-blue-200 shadow-sm relative">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">
                ...
                {isDropdown && isLoading && (
                    <span className="ml-2 text-blue-400 text-xs font-normal">
                        (Loading...)
                    </span>
                )}
            </label>
...
// PaymentInput component (unchanged)
const PaymentInput = ({ label, value, color = "gray" }) => {
    const displayValue = formatAmount(value).toLocaleString("en-IN", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
    });
    const inputStyle = `w-full text-lg font-semibold border-b-2 outline-none 
        py-1 transition duration-150 cursor-default
        ${color === "green" 
            ? "border-green-300 text-green-700 bg-gray-50" 
            : "border-red-300 text-red-700 bg-gray-50"}`;
    return (
        <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1">
                {label}
...
const InfoField = ({ label, value, isMoney = false, colorClass = "text-gray-800" }) => (
    <div className="flex flex-col border-b border-gray-100 pb-1">
        ...
        <span 
            className={`text-sm font-medium ${colorClass} truncate`} 
            title={isMoney 
...
const DocLink = ({ label, url }) => {
    ...
    return (
        <a 
            ...
            target="_blank" 
            rel="noreferrer" 
            className="flex items-center gap-1 text-xs bg-blue-50 text-blue-600 
                px-2 py-1 rounded hover:bg-blue-100 border border-blue-200 transition"
        >
            <FileText size={12} /> {label}
        </a>
    );
...
const Payment_Tally = () => {
    ...
    // ✅ NEW: Bank names API call
    const { 
        data: bankNamesFromAPI = [], 
        isLoading: isBankNamesLoading,
    ...
    // ✅ Dynamic bank options from API + fallback static options
    ...
    // ✅ API se aaye banks ko use karo, error pe static fallback
    const bankOptions = useMemo(() => {
        if (isBankNamesError || bankNamesFromAPI.length === 0) {
            // Fallback to static options if API fails or empty
            return staticBankOptions;
        }
        // API data use karo - unique merge with static if needed
        return bankNamesFromAPI;
    }, [bankNamesFromAPI, isBankNamesError]);
    ...
    const handleFetch = () => {
        if (selectedFirm && selectedContractor) {
            const filtered = bills.filter(
                b => b.firmName === selectedFirm && 
                     b.contractorName === selectedContractor
            );
            setFilteredBills(filtered);
            setShowData(true);
            setSelectedBillIds([]);
    ...
    const getNumericValue = (billId, field) => 
        formatAmount(billInputData[billId]?.[field]);
    ...
    const calculatedBillsData = useMemo(() => {
        ...
        filteredBills.forEach(bill => {
            ...
            let tdsAmount = latestTDS > 0 ? 0 : getNumericValue(uniqueId, 'tds_amount_8');
            const currentPaidAmount = getNumericValue(uniqueId, 'paid_amount_8');
            const roundUpValue = parseFloat(inputs.round_up || "0") || 0;

            let payableAmount = latestBalance > 0 
                ? latestBalance 
                : billAmount - tdsAmount;
            ...
            data[uniqueId] = {
                ...
                roundUpValue,
                totalAllAmounts,
                paidAmount8Value: hasAnyPreviousData ? payableAmount : totalPaid,
                hasAnyPreviousData,
                hasPreviousTDS: latestTDS > 0,
                hasPreviousPayment: latestPaid > 0,
                hasPreviousBalance: latestBalance > 0,
    ...
    const handleSubmitAll = async () => {
        if (!globalPaymentData.BANK_DETAILS_8 || 
            !globalPaymentData.PAYMENT_MODE_8 || 
            !globalPaymentData.PAYMENT_DATE_8) {
            alert("Please fill all Global Payment Details (Bank, Mode, Date)");
            return;
        }
        ...
        if (emptyPaidBills.length > 0) {
            alert("Please enter a valid Current Paid Amount (8) for all selected bills.");
            return;
        }
        ...
        const payload = selectedBillIds.map(billId => {
            const bill = filteredBills.find(b => (b._id || b.rccBillNo) === billId);
            const calc = calculatedBillsData[billId] || {};
            const hasAnyPreviousData = calc.latestPaid > 0 || 
                                       calc.latestTDS > 0 || 
                                       calc.latestBalance > 0;
        ...
        try {
            await updatePayments(payload).unwrap();
            alert(
                `Success! ${payload.length} bills processed!\n` +
                `Grand Total: ₹${grandTotalValue.toLocaleString("en-IN", { 
                    minimumFractionDigits: 2 
                })}`
            );
            handleClearFilter();
                    ...
                    {!showData ? (
                        ...
                        <div className="max-w-[1600px] mx-auto">
                            ...
                            <div className="bg-white p-4 rounded-lg shadow mb-6 flex 
                                justify-between items-center sticky top-2 z-10 border border-gray-300">
                                <div>
                                    <div className="text-gray-500 text-sm">Viewing Bills for:</div>
                                    <span className="font-bold text-gray-900 ml-2 text-xl">
                                        {selectedContractor}
                                    </span>
                                    <span className="mx-2 text-gray-300">|</span>
                                    <span className="text-gray-600 font-medium">{selectedFirm}</span>
                            ...
                            <div className="space-y-6">
                                {filteredBills.map((bill) => {
                                    ...
                                    const hasPreviousTDS = formatAmount(bill.latestTDSAmount8) > 0;
                                    const hasPreviousBalance = formatAmount(bill.latestBalanceAmount8) > 0;
                                    const hasAnyPreviousData = hasPreviousPayment || 
                                                               hasPreviousTDS || 
                                                               hasPreviousBalance;
                                    const totalAllAmounts = billCalculations.latestPaid + 
                                                           billCalculations.currentPaidAmount + 
                                                           billCalculations.latestTDS;
                                    const isBalanceZeroCondition = Math.abs(
                                        totalAllAmounts - billCalculations.billAmount
                                    ) <= 0.01;
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className={`p-4 flex items-center justify-between 
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <button 
                                                        onClick={() => toggleBillSelection(uniqueId)} 
                                                        className={`flex items-center gap-2 px-4 py-2 
                                                            rounded-lg font-bold shadow-sm border 
                                                            transition-all 
                                                            ${isSelected 
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-gray-500 font-mono">
                                                            Bill No: {bill.rccBillNo}
                                                        </p>
                                                        {hasAnyPreviousData && (
                                                            <div className="flex items-center gap-2 mt-1">
                                                                {hasPreviousPayment && (
                                                                    <span className="text-xs bg-yellow-100 
                                                                        text-yellow-800 px-2 py-0.5 rounded">
                                                                        Prev Paid: ₹{formatAmount(bill.latestPaidAmount8)
                                                                            .toLocaleString("en-IN")}
                                                                    </span>
                                                                )}
                                                                {hasPreviousTDS && (
                                                                    <span className="text-xs bg-purple-100 
                                                                        text-purple-800 px-2 py-0.5 rounded">
                                                                        Prev TDS: ₹{formatAmount(bill.latestTDSAmount8)
                                                                            .toLocaleString("en-IN")}
                                                                    </span>
                                                                )}
                                                                {hasPreviousBalance && (
                                                                    <span className="text-xs bg-blue-100 
                                                                        text-blue-800 px-2 py-0.5 rounded">
                                                                        Prev Balance: ₹{formatAmount(bill.latestBalanceAmount8)
                                                                            .toLocaleString("en-IN")}
                                                                    </span>
                                                                )}
                                                            </div>
                                            ...
                                            <div className="p-5">
                                                <div className="grid grid-cols-2 md:grid-cols-4 
                                                    lg:grid-cols-6 gap-x-6 gap-y-4 mb-4">
                                                    <InfoField label="Contractor Bill No" value={bill.contractorBillNo} />
                                                    <InfoField label="Work Name" value={bill.workName} />
                                                    <InfoField label="Contractor Name" value={bill.contractorName} />
                                                    <InfoField label="Firm Name" value={bill.firmName} />
                                                    <InfoField label="Project ID" value={bill.projectId} />
                                                    <InfoField label="Work Order No" value={bill.WorkOrderNo} />
                                                    <InfoField label="Work Order Value" value={bill.WorkOrderValue} isMoney={true} />
                                                ...
                                                <div className="flex flex-wrap gap-3 items-center 
                                                    ...
                                                    <span className="text-xs font-bold text-gray-400 
                                                        uppercase mr-2">Document Links:</span>
                                                    <DocLink label="Bill PDF" url={bill.billUrl} />
                                                </div>
                                            </div>

                                            {/* Payment Inputs (shown when selected) */}
                                            {isSelected && (
                                                <div className="bg-blue-50 border-t-2 border-blue-200 p-6">
                                                    <h3 className="text-blue-800 font-bold mb-4">
                                                        Local Payment Inputs for Bill: {bill.rccBillNo}
                                                    </h3>
                                                            ...
                                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                                {hasPreviousPayment && (
                                                                    <div className="text-center">
                                                                        ...
                                                                        <div className="text-lg font-bold text-blue-700">
                                                                            ₹{formatAmount(bill.latestPaidAmount8)
                                                                                .toLocaleString("en-IN")}
                                                                        </div>
                                                                    </div>
                                                                )}
                                                                {hasPreviousBalance && (
                                                                    <div className="text-center">
                                                                        ...
                                                                        <div className="text-lg font-bold text-red-700">
                                                                            ₹{formatAmount(bill.latestBalanceAmount8)
                                                                                .toLocaleString("en-IN")}
                                                                        </div>
                                                                    </div>
                                                                )}
                                                                {hasPreviousTDS && (
                                                                    <div className="text-center">
                                                                        ...
                                                                        <div className="text-lg font-bold text-purple-700">
                                                                            ₹{formatAmount(bill.latestTDSAmount8)
                                                                                .toLocaleString("en-IN")}
                                                                        </div>
                                                                    </div>
                                                                )}
                                                    ...
                                                    <div className="grid grid-cols-1 md:grid-cols-2 
                                                        lg:grid-cols-5 gap-6 items-end">
                                                        <PaymentInputComponent
                                                            label="TDS Amount (8)"
                                                            id={uniqueId}
                                                            initialValue={inputs.tds_amount_8}
                                                            ...
                                                            color="blue"
                                                            isNumeric={true}
                                                            placeholder={hasPreviousTDS 
                                                                ? "0 (Previous TDS Applied)" 
                                                                : "Enter TDS Amount"}
                                                            readOnly={hasPreviousTDS}
                                                        />
                                                        <PaymentInput
                                                        ...
                                                        <PaymentInput
                                                            label="New Balance Amount"
                                                            value={billCalculations.displayBalance ?? 0}
                                                            color={billCalculations.displayBalance === 0 
                                                                ? "green" : "red"}
                                                        />
                                                    </div>

                                                    {isBalanceZeroCondition && 
                                                    ...
                                                    <div className="mt-6">
                                                        <button
                                                            ...
                                                            className="w-full py-3 bg-indigo-600 text-white 
                                                                font-bold rounded-lg hover:bg-indigo-700 transition"
                                                        >
                                                            Save & Proceed to Global Payment
                                                        </button>
                                                    </div>
                            ...
                            {selectedBillIds.length > 0 && (
                                <div className="bg-green-50 p-6 rounded-xl shadow-lg 
                                    border-2 border-green-400 my-8">
                                    <div className="flex justify-between items-center">
                                        <h2 className="text-2xl font-bold text-green-800">
                                            Grand Total (Current Paid Amount)
                                        </h2>
                                        <div className="text-4xl font-extrabold text-green-700">
                                            ₹{grandTotalCurrentPaidAmount.toLocaleString("en-IN", { 
                                                minimumFractionDigits: 2 
                                            })}
                                        </div>
                                    </div>
                                    <p className="text-green-600 mt-1">
                                        Sum of <strong>Current Paid Amount</strong> from{" "}
                                        {selectedBillIds.length} selected bills.
                                    </p>
                                </div>
                            )}

                            {/* ✅ Global Payment Details with Dynamic Bank Dropdown */}
                            {selectedBillIds.length > 0 && (
                                <div className="mt-8 mb-12" ref={globalPaymentRef}>
                                    <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-2">
                                        <span className="text-blue-600">Global Payment Details</span>{" "}
                                        ({selectedBillIds.length} Bills Selected)
                                    </h2>
                                    
                                    {isSuccess && (
                                        <div className="mb-4 p-4 bg-green-100 border 
                                            border-green-400 text-green-800 rounded-lg">
                                            Payment submitted successfully!
                                        </div>
                                    )}
                                    {isError && (
                                        <div className="mb-4 p-4 bg-red-100 border 
                                            border-red-400 text-red-800 rounded-lg">
                                            Error: {error?.data?.error || "Submission failed"}
                                        </div>
                                    )}

                                    {/* ✅ Bank API Error Warning */}
                                    {isBankNamesError && (
                                        <div className="mb-4 p-3 bg-yellow-50 border 
                                            border-yellow-300 text-yellow-700 rounded-lg text-sm">
                                            ⚠️ Bank names could not be loaded from server. 
                                            Using default bank list.
                                        </div>
                                    )}
                                    ...
                                    <div className="bg-white p-6 rounded-xl shadow-lg 
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 
                                            ...
                                            <PaymentInputComponent
                                                label="BANK DETAILS (8)"
                                                id="BANK_DETAILS_8"
                                                initialValue={globalPaymentData.BANK_DETAILS_8}
                                                onValueChange={(id, v) => handleGlobalInputChange(id, v)}
                                                color="green"
                                                options={bankOptions}          // ← API se aaya data
                                                isLoading={isBankNamesLoading} // ← Loading state
                                            />
                                            
                                            <PaymentInputComponent
                                                label="PAYMENT MODE (8)"
                                                id="PAYMENT_MODE_8"
                                                initialValue={globalPaymentData.PAYMENT_MODE_8}
                                                onValueChange={(id, v) => handleGlobalInputChange(id, v)}
                                                color="green"
                                                options={modeOptions}
                                            />
                                            <PaymentInputComponent
                                                ...
                                                id="PAYMENT_DETAILS_8"
                                                initialValue={globalPaymentData.PAYMENT_DETAILS_8}
                                                onValueChange={(id, v) => handleGlobalInputChange(id, v)}
                                                color="green"
                                                isNumeric={false}
                                            />
                                            <PaymentInputComponent
                                                ...
                                                id="PAYMENT_DATE_8"
                                                initialValue={globalPaymentData.PAYMENT_DATE_8}
                                                onValueChange={(id, v) => handleGlobalInputChange(id, v)}
                                                color="green"
                                                isDate={true}
                                                isNumeric={false}
                                        ...
                                        <div className="mt-8 flex justify-end">
                                            <button
                                                ...
                                                className="bg-green-600 hover:bg-green-700 text-white 
                                                    px-8 py-3 rounded-lg font-bold shadow-lg 
                            ...
                            {selectedBillIds.length === 0 && filteredBills.length > 0 && (
                                <div className="bg-yellow-50 p-4 text-center rounded-lg 
                                    border border-yellow-300 text-yellow-800 font-medium mt-8">
                                    Please select at least one bill to proceed.
                                </div>
                            )}
                        </div>
