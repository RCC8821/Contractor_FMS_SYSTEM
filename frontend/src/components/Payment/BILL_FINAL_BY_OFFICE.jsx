...
const BILL_FINAL_BY_OFFICE = () => {
  ...
  const findPreviousBillData = (
    ...
    const matchingBills = doneBills.filter(doneBill => {
      ...
      const isProjectMatch = String(doneBill.projectId || "").trim() === String(currentProjectId || "").trim();
      const isContractorMatch = String(doneBill.contractorName || "").trim().toLowerCase() === String(currentContractor || "").trim().toLowerCase();
      const isFirmMatch = String(doneBill.firmName || "").trim().toLowerCase() === String(currentFirm || "").trim().toLowerCase();
      const isWorkMatch = String(doneBill.workName || "").trim().toLowerCase() === String(currentWorkType || "").trim().toLowerCase();
      
      // ✅ Work order comparison (optional, lekin agar available ho toh check karein)
      let isWorkOrderMatch = true;
      if (currentWorkOrderNo && currentWorkOrderNo !== "" && currentWorkOrderNo !== " ") {
        isWorkOrderMatch = String(doneBill.WorkOrderNo || "").trim() === String(currentWorkOrderNo || "").trim();
      }

      // ✅ Sab conditions true hone chahiye
      const isMatch = isProjectMatch && isContractorMatch && isFirmMatch && isWorkMatch && isWorkOrderMatch;
    ...
    console.log("📋 Strict Matching Previous Bills Found:", matchingBills.length);
  ...
  const handleSearchBillSelect = (bill) => {
    ...
    if (bill.contractorName && bill.firmName && bill.workName && bill.projectId) {
  ...
  const handleSave = async () => {
    ...
    console.log("materialDebitAmount5 (should go to Column Z):", payload.materialDebitAmount5);
                  ...
                  <div className="p-4 sm:p-6 md:p-8 max-h-[85vh] sm:max-h-screen overflow-y-auto">
                    <div className="space-y-4 sm:space-y-6 md:space-y-8">
                            ...
                            <div className="bg-gradient-to-r from-gray-100 to-gray-200 p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-gray-300 shadow-lg">
                              ...
                              <div className="text-center p-4">
                                <p className="text-gray-600 text-sm sm:text-base">
                                  <span className="font-bold">Note:</span> Previous bill information will appear only when you select <span className="font-bold text-blue-600">Contractor, Firm, and Work Type</span> that match with a previous bill having the <span className="font-bold text-blue-600">same Project ID</span>.
                                </p>
                                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-left">
                                  <div className="bg-white p-3 rounded-lg border">
                                    <span className="font-bold text-gray-700">Required Fields:</span>
                                    <ul className="mt-2 text-sm text-gray-600 space-y-1">
                                      <li className={`flex items-center ${formData.contractorName ? 'text-green-600' : 'text-red-500'}`}>
                                        {formData.contractorName ? '✓' : '✗'} Contractor Name
                                      </li>
                                      <li className={`flex items-center ${formData.firmName ? 'text-green-600' : 'text-red-500'}`}>
                                        {formData.firmName ? '✓' : '✗'} Firm Name
                                      </li>
                                      <li className={`flex items-center ${formData.workType ? 'text-green-600' : 'text-red-500'}`}>
                                        {formData.workType ? '✓' : '✗'} Work Type
                                      </li>
                                      <li className={`flex items-center ${selectedBill?.projectId ? 'text-green-600' : 'text-red-500'}`}>
                                        {selectedBill?.projectId ? '✓' : '✗'} Project ID ({selectedBill?.projectId})
                                      </li>
                                    </ul>
                                  </div>
                                  <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                                    <span className="font-bold text-blue-700">Matching Criteria:</span>
                          ...
                          return (
                            <div className="bg-gradient-to-r from-amber-50 to-orange-50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-amber-400 shadow-lg">
                              <h3 className="text-base sm:text-lg md:text-xl font-bold text-amber-900 mb-3 sm:mb-4">
                                📋 Previous Bill Found (RCC No: {previousBillData.rccBillNo})
                              </h3>
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                                ...
                                <div className="bg-white p-3 sm:p-4 rounded-xl border border-amber-300">
                                  <label className="block text-xs sm:text-sm font-bold text-amber-700 mb-2">Project ID</label>
                                  <p className="text-base sm:text-lg md:text-xl font-black text-amber-900 break-all">
                              ...
                              <div className="mt-3 text-xs text-amber-700 font-semibold">
                                ✅ Showing data from bill no: {previousBillData.rccBillNo} (Current bill: {selectedBill.rccBillNo}) - <span className="text-green-600">All fields match ✓</span>
                              </div>
                            </div>
                          );
                        }
                          ...
                          <div className="bg-gradient-to-r from-red-50 to-pink-100 p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-red-300 shadow-lg">
                            ...
                            <div className="text-center p-3">
                              <p className="text-red-700 text-sm sm:text-base mb-4">
                                No previous bill found with <span className="font-bold">exact matching criteria</span> for:
                              </p>
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                <div className="bg-white p-3 rounded-lg border">
                                  <span className="font-bold text-gray-600">Project ID:</span>
                                  <p className="font-bold text-gray-800">{selectedBill.projectId}</p>
                                </div>
                                <div className="bg-white p-3 rounded-lg border">
                                  <span className="font-bold text-gray-600">Contractor:</span>
                                  <p className="font-bold text-gray-800">{formData.contractorName}</p>
                                </div>
                                <div className="bg-white p-3 rounded-lg border">
                                  <span className="font-bold text-gray-600">Firm:</span>
                                  <p className="font-bold text-gray-800">{formData.firmName}</p>
                                </div>
                                <div className="bg-white p-3 rounded-lg border">
                                  <span className="font-bold text-gray-600">Work Type:</span>
                                  <p className="font-bold text-gray-800">{formData.workType}</p>
                                </div>
                              </div>
                              <p className="mt-4 text-xs text-gray-600">
                                Previous bill section will only appear when all fields exactly match a previous bill with smaller RCC number.
                              </p>
                            </div>
                          </div>
                        );
                      ...
                      <div className="bg-red-50 p-4 sm:p-5 rounded-xl border-2 border-red-300">
                        <label className="block text-xs sm:text-sm font-bold text-red-700 mb-2"> Other Debit Amount (Without Material)</label>
                      ...
                      <div className="bg-gradient-to-br from-emerald-600 to-green-700 text-white p-6 sm:p-8 rounded-xl text-center">
                        <p className="text-base sm:text-lg font-bold mb-2">
                          PAID AMOUNT 
                          <br />
                          <span className="text-sm">(After SD & Material Debit)</span>
                        </p>
                      ...
                      <div className="bg-indigo-50 p-4 sm:p-5 rounded-xl border-2 border-indigo-300">
                        <label className="block text-xs sm:text-sm font-bold text-indigo-700 mb-2">
                          Up To Date Paid Amount 
                          <span className="text-xs text-gray-600 ml-2">(Previous + Current Bill + GST)</span>
                        </label>
