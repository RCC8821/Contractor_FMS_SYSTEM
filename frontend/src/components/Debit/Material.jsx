...
// ─── PDF Actions Modal ─────────────────────────────────────────────────────────
// Shows after successful PDF generation: View / Download / Print
const PdfActionsModal = ({ pdfResult, onClose }) => {
  ...
  const handleDownload = () => {
    ...
    link.href = pdfUrl;
    link.download = `Debit_${pdfResult?.debitUID || 'PDF'}.pdf`;
      ...
      >
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-green-100 text-sm">Your debit PDF is ready</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-5 border-b border-gray-100">
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            {pdfResult?.totalMaterials && (
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-gray-400 text-xs uppercase tracking-wide mb-1">Materials</p>
                <p className="font-semibold text-gray-800">{pdfResult.totalMaterials}</p>
              </div>
            )}
            {pdfResult?.totalPOs && (
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-gray-400 text-xs uppercase tracking-wide mb-1">POs</p>
                <p className="font-semibold text-gray-800">{pdfResult.totalPOs}</p>
              </div>
            )}
            {pdfResult?.dateTime && (
...
const Material = () => {
  ...
  // ── NEW: stores the API result after successful PDF generation ──────────
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await submitDebit({
        selectedRows,
        status,
        totalAmount: selectedTotals.rawTotalAmount,
      }).unwrap();

      console.log('Debit Success:', result);

      // ── Close the confirm modal, store result → show PDF actions modal ──
      setShowModal(false);
      setPdfResult(result?.data || result);   // handle both {data: {...}} and flat response
      setSelectedRows([]);
      refetch();
    } catch (error) {
      console.error('Debit Error:', error);
  ...
  // Close PDF modal and reset result
