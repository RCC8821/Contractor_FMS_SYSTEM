...
//       spreadsheetId,
//       range: 'Contractor_Billing_FMS!A7:U', // Updated sheet name
//     });
...
//       .filter(row => row[19] && !row[20]) // planned2 yes (col S), actual2 no (col T)
//       .map(row => ({
//         planned2: row[19] || '',
//         UID: row[1] || '',
//         projectId: row[2] || '',
...
//         amount: row[12] || '',
//         billNo: row[13] || '',
//         rccBillNo: row[14] || '',           // NEW: Rcc Bill No from column O
//         billDate: row[15] || '',            // Warning: Ab yeh shift ho gaya? Wait!
//         billUrl: row[16] || '',
//         prevBillUrl: row[17] || '',
//         remark: row[18] || '',
//       }));
...
//     const response = await sheets.spreadsheets.values.get({
//       spreadsheetId,
//       range: 'Project_Data!A:F', // A to F covers all needed columns
//     });
...
//     // Map each row starting from index 1 (skip header)
//     const data = rows.slice(1).map(row => ({
//       Project_ID: (row[0] || '').trim(),           // Column A
//       Project_Name: (row[1] || '').trim(),         // Column B → NEW
//       Contractor_Name: (row[4] || '').trim(),      // Column E
//       Contractor_Firm_Name: (row[5] || '').trim(), // Column F → NEW
//     }));
...
//       // === V:AC VALUES (NO T COLUMN UPDATE) ===
//       const values = [
//         status || '',                    // V
//         '',                              // W
...
//     res.json({
//       success: true,
//       message: 'Smart bulk update completed (NO T=Yes)',
